...
if(($y>=1)||($m>=1)||($d>=1)||($h>=12))
{
    ...
    for($i=0;$i<9;$i++)
    {
        if($counts[$i]!=0)
        {
            $sql="UPDATE `required` SET `count`=$counts[$i] WHERE `date`='$date' AND `session`='$session' AND `dept_id`=$dept[$i]";
            //echo $sql="INSERT INTO `required`(`date`, `dept_id`, `session`, `count`) VALUES ('$date',$dept[$i],'$session',$counts[$i]);";
            $result=mysql_query($sql,$conn);
            $sql="UPDATE required_reserve` SET `count`=$counts[$i] WHERE `date`='$date' AND `session`='$session' AND `dept_id`=$dept[$i]";
...
{
header("location:admin_req.php?s=3&d=$date");
}
//print_r($result);
?>
